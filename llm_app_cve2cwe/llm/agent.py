...
class FinalCWEAssignment(BaseModel):
    cwe_id: str = Field(description="The specific ID, e.g., 'CWE-564'")
    confidence: Literal["High", "Medium", "Low"]
    logic_chain: str = Field(description="Why this specific CWE over the generic CWE-89?")
    ...
    def run(self, prompt: str) -> str:
        ...
        for step in range(1, self._max_steps + 1):
            ...
            if step == self._max_steps:
                 messages.append({
                     "role": "system",
                     "content": "You have reached the maximum number of steps. You MUST now call 'provide_cwe_match' with your best current conclusion.",
                 })

            rendered = self._render_messages(messages)
            response = self._llm.complete(rendered)
            if self._verbose:
                print(f"STEP {step} MODEL_RESPONSE: {response!r}")
            ...
            if "provide_cwe_match" in tool_names and len(tool_requests) > 1:
                messages.append({
                    "role": "system", 
                    "content": "Error: You cannot call 'provide_cwe_match' simultaneously with other tools. You must wait for other tool results before providing the final answer. If you are ready to answer, call only 'provide_cwe_match'."
                })
                continue
            ...
            for idx, tool_request in enumerate(tool_requests, start=1):
                ...
                if tool_name == "provide_cwe_match":
                    if self._verbose:
                        print(f"DEBUG: tool_history={tool_history}")
                    if "get_cwe_information" not in tool_history:
                         # Soft enforcement: If they claim to know the answer but haven't checked details, warn them.
                         # Unless confidence is Low? No, let's enforce it to ensure quality.
                         warn_msg = "You must call 'get_cwe_information' for your candidate CWE to verify details before finalizing."
                         messages.append({
                             "role": "system", 
                             "content": warn_msg
                         })
                         if self._verbose:
                             print(f"DEBUG: Rejected provide_cwe_match. Message: {warn_msg}")
                         continue
                ...
                if self._verbose:
                    print(
                        f"STEP {step} TOOL_CALL {idx}/{len(tool_requests)}:",
                ...
                if self._verbose:
                    print(
                        f"STEP {step} TOOL_RESULT {idx}/{len(tool_requests)}:",
